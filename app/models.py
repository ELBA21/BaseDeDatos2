...
class Partido(Base):
    ...
    # equipo-partido
    equipo1 = relationship(
        "Equipo", back_populates="partido_equipo1", foreign_keys=[equipo1_id]
    )
    equipo2 = relationship(
        "Equipo", back_populates="partido_equipo2", foreign_keys=[equipo2_id]
    )
    equipo_ganador = relationship(
        "Equipo",
        back_populates="partido_equipo_ganador",
        foreign_keys=[equipo_ganador_id],
    )
    #
    ##partido-jugador
    jugador1 = relationship(
        "Jugador", back_populates="partido_jugador1", foreign_keys=[jugador1_id]
    )
    jugador2 = relationship(
        "Jugador", back_populates="partido_jugador2", foreign_keys=[jugador2_id]
    )
    jugador_ganador = relationship(
        "Jugador",
        back_populates="partido_jugador_ganador",
        foreign_keys=[jugador_ganador_id],
    )
    #
    # categoria-partido
...
class Equipo(Base):
    ...
    # jugador-equipo
    jugador1 = relationship(
        "Jugador", back_populates="equipos_jugador1", foreign_keys=[jugador_id]
    )
    jugador2 = relationship(
        "Jugador", back_populates="equipos_jugador2", foreign_keys=[jugador2_id]
    )
    ...
    # partido-equipo
    partido_equipo1 = relationship(
        "Partido", back_populates="equipo1", foreign_keys=[Partido.equipo1_id]
    )
    partido_equipo2 = relationship(
        "Partido", back_populates="equipo2", foreign_keys=[Partido.equipo2_id]
    )
    partido_equipo_ganador = relationship(
        "Partido",
        back_populates="equipo_ganador",
        foreign_keys=[Partido.equipo_ganador_id],
    )
...
class Jugador(Base):
    ...
    equipos_jugador1 = relationship(
        "Equipo", back_populates="jugador1", foreign_keys=[Equipo.jugador_id]
    )
    equipos_jugador2 = relationship(
        "Equipo", back_populates="jugador2", foreign_keys=[Equipo.jugador2_id]
    )

    # partido-jugador
    partido_jugador1 = relationship(
        "Partido", back_populates="jugador1", foreign_keys=[Partido.jugador1_id]
    )
    partido_jugador2 = relationship(
        "Partido", back_populates="jugador2", foreign_keys=[Partido.jugador2_id]
    )
    partido_jugador_ganador = relationship(
        "Partido",
        back_populates="jugador_ganador",
        foreign_keys=[Partido.jugador_ganador_id],
    )
...
class Torneo(Base):
    ...
    torneo_categorias = relationship(
        "TorneoCategoria", back_populates="torneo"
    )  # ELANDRE
...
class Ciudad(
    Base
):  # Ciudad contiene a pais porque no vamos a estar en puerto montt en francia
