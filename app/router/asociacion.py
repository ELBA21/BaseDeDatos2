...
from ..crud.asociacion import (
    get_asociacion_id,
    create_asociacion,
    update_asociacion_id,
    delete_asociacion,
)
...
def create_asociacion_endpoint(
    nombre: str, ciudad: int, session: Session = Depends(get_db)
):
    asociacion = create_asociacion(session, nombre, ciudad)
    return {
        "id": asociacion.id,
...
def update_asociacion_id_endpoint(
    asociacion_id: int,
