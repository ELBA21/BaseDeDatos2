...
def create_categoria(session: Session, edad_Min:int,edad_Max:int, genero:str,set_por_partido:int, puntos_por_set:int):
    ...
    categoria = Categoria(edad_Min=edad_Min, edad_Max=edad_Max, genero=genero, set_por_partido=set_por_partido, puntos_por_set=puntos_por_set)
...
def update_categoria_id(session: Session, categoria_id: int, edad_Min: Optional[int] = None, edad_Max: Optional[int] = None, genero: Optional[str] = None, set_por_partido: Optional[int] = None, puntos_por_set: Optional[int] = None):
    categoria = session.get(Categoria, categoria_id)
    if not categoria:
        raise ValueError("Categoria no encontrada")
